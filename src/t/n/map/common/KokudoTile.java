...
//		computeScale();
	...
	public KokudoTile(int zoomLevel, int tileNoX, int tileNoY) {
		...
		scale = TileUtil.computeScale(zoomLevel);
		...
		if(isError) {
			...
			super.tileNoX = tileNoX;
			super.tileNoY = tileNoY;
			computeLonLatOfCorners();
			this.lonlat = getLonLat();
		}
	}
...
//		super(zoomLevel, lonlat);
	...
	public int getPixelOffsetX() {
		return getPixelCoordX(lonlat.getLongitude());
	}

	public int getPixelOffsetY() {
		return getPixelCoordY(lonlat.getLatitude());
	}

	//lat(緯度, 北緯は正、南緯は負)は大きいほうをUpper,lon(経度, 東経は正、西経は負で表す)は大きいほうをrightとする。
